...
function FriendsScreen({ tweaks, theme, nav }) {
  const friends = [
  ...
  const max = friends[0].steps;

  return (
    <div data-screen-label="06 Friends" style={{
        ...
        <Card theme={theme} padding={0} style={{
          ...
          <div style={{ padding: '16px 18px', color: theme.bg }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ ...TYPE.sans, fontSize: 11, letterSpacing: '0.16em',
                  textTransform: 'uppercase', opacity: 0.75 }}>Active challenge</div>
                <div style={{ ...TYPE.display, fontSize: 22, marginTop: 4 }}>May Mileage</div>
                <div style={{ ...TYPE.sans, fontSize: 12, opacity: 0.85, marginTop: 2 }}>
                  3 days left · 7 friends competing
                </div>
              </div>
            ...
            <div style={{ marginTop: 14, height: 4, background: 'rgba(0,0,0,0.18)', borderRadius: 2 }}>
              <div style={{ width: '64%', height: '100%', background: theme.bg,
                borderRadius: 2, opacity: 0.9 }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6,
              ...TYPE.mono, fontSize: 11, opacity: 0.85 }}>
      ...
      <div style={{ padding: '0 16px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, justifyContent: 'center',
          ...
          {[friends[1], friends[0], friends[2]].map((f, i) => {
            ...
            return (
              <div key={f.rank} style={{ flex: 1, display: 'flex', flexDirection: 'column',
                alignItems: 'center', gap: 6, maxWidth: 90 }}>
                <div style={{
                ...
                <div style={{ ...TYPE.sans, fontSize: 11, color: theme.text, textAlign: 'center',
                  fontWeight: 500, maxWidth: 80, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {f.you ? 'You' : f.name}
                </div>
                <div style={{ ...TYPE.mono, fontSize: 11, color: theme.textDim }}>
                  {(f.steps/1000).toFixed(1)}k
                </div>
                <div style={{
                  width: '100%', height: heights[place],
                  background: place === 1 ? theme.accent
                    : place === 2 ? theme.surface : theme.surfaceAlt,
        ...
        <Card theme={theme} padding={0}>
          {friends.map((f, i) => (
            <div key={f.rank} style={{
              ...
              <div style={{ ...TYPE.mono, fontSize: 13,
                color: f.rank <= 3 ? theme.accent : theme.textMuted,
                fontWeight: f.rank <= 3 ? 600 : 400 }}>
                {f.rank}
              </div>
              <div style={{
              ...
              <div>
                <div style={{ ...TYPE.sans, fontSize: 13, color: theme.text,
                  fontWeight: f.you ? 600 : 500 }}>{f.name}</div>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ ...TYPE.mono, fontSize: 13, color: theme.text }}>
                  {f.steps.toLocaleString()}
                </div>
                {f.change !== 0 && (
                  <div style={{ ...TYPE.mono, fontSize: 10, marginTop: 1,
                    color: f.change > 0 ? theme.accent : theme.rose,
          ...
          {[
            ...
            <div key={i} style={{
              ...
              <div style={{ width: 32, height: 32, borderRadius: '50%',
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                ...TYPE.sans, fontSize: 12, fontWeight: 600 }}>{a.who[0]}</div>
