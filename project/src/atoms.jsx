  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, animation: animate ? 'breathe 6s ease-in-out infinite' : 'none' }}>
      ...
      <div style={{
        ...
        {label && <div style={{ ...TYPE.sans, fontSize: 11, letterSpacing: '0.18em',
          color: theme.textDim, textTransform: 'uppercase', marginBottom: 6 }}>Today's steps</div>}
        <div style={{ ...TYPE.display, fontSize: 64, lineHeight: 1, color: theme.text }}>
          {steps.toLocaleString()}
        </div>
        <div style={{ ...TYPE.mono, fontSize: 12, color: theme.textDim, marginTop: 10,
...
// ── Stat card (compact) ─────────────────────────────────────────────────
function StatCard({ label, value, unit, sub, icon, theme, accent }) {
  return (
    <div style={{
...
// ── App bar (custom, replaces android frame's app bar) ──────────────────
function AppBar({ title, subtitle, theme, leading, trailing, large = false }) {
  return (
    <div style={{
      ...
      {large && (
        <div>
          {subtitle && <div style={{ ...TYPE.sans, fontSize: 11, letterSpacing: '0.18em',
            color: theme.textDim, textTransform: 'uppercase', marginBottom: 4 }}>{subtitle}</div>}
...
// ── Icon button ─────────────────────────────────────────────────────────
function IconButton({ children, onClick, theme, variant = 'ghost', size = 36 }) {
  const styles = {
  ...
  return (
    <div style={{
      ...
      <div style={{ ...TYPE.sans, fontSize: 11, letterSpacing: '0.2em',
        color: theme.textDim, textTransform: 'uppercase', fontWeight: 500 }}>{title}</div>
...
// ── Pill ────────────────────────────────────────────────────────────────
function Pill({ children, theme, tone = 'default', icon }) {
  const tones = {
...
// ── Linear progress ─────────────────────────────────────────────────────
function LinearProgress({ value, max = 100, theme, color, height = 6 }) {
  const pct = Math.min(value/max, 1) * 100;
  return (
    <div style={{
      width: '100%', height, borderRadius: height,
      background: theme.borderStrong, overflow: 'hidden',
    }}>
      <div style={{
...
// ── Sparkline ───────────────────────────────────────────────────────────
function Sparkline({ data, width = 100, height = 32, color, theme }) {
...
function BarChart({ data, labels, theme, height = 140, highlight = -1, goal }) {
      ...
      {goal && (
        <div style={{
          ...
          <span style={{
            ...
            ...TYPE.mono, fontSize: 9, color: theme.textMuted,
          }}>{(goal/1000).toFixed(0)}k goal</span>
        </div>
      )}
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
          {labels.map((l, i) => (
            <div key={i} style={{ flex: 1, textAlign: 'center',
              ...TYPE.sans, fontSize: 10,
              color: i === highlight ? theme.accent : theme.textMuted,
              fontWeight: i === highlight ? 600 : 400 }}>{l}</div>
          ))}
        </div>
      )}
    </div>
...
Object.assign(window, {
  StepRing, StatCard, BottomNav, AppBar, IconButton, SectionHeader,
  Pill, LinearProgress, Card, Sparkline, BarChart,
});
