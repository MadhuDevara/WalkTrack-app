...
function AchievementsScreen({ tweaks, theme, nav }) {
  const badges = [
  ...
  return (
    <div data-screen-label="05 Achievements" style={{
        ...
        <Card theme={theme} padding={0} style={{
          ...
          <div style={{ padding: '22px 22px 18px', position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ ...TYPE.sans, fontSize: 11, letterSpacing: '0.18em',
                color: theme.warm, textTransform: 'uppercase', fontWeight: 600 }}>Active streak</span>
            </div>
            ...
            <div style={{ ...TYPE.sans, fontSize: 12, color: theme.textDim, marginTop: 6 }}>
              Personal best · 2 days to a new record
            </div>
            {/* Calendar dots */}
              ...
              {Array.from({ length: 30 }).map((_, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    background: today ? theme.warm
                      : (filled ? theme.accent : theme.borderStrong),
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ ...TYPE.sans, fontSize: 11, letterSpacing: '0.16em',
                color: theme.textMuted, textTransform: 'uppercase' }}>Level 12 · Trailblazer</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          {badges.map(b => (
            <div key={b.id} style={{
              ...
              <div style={{ ...TYPE.serif, fontSize: 13, color: theme.text, marginTop: 8,
                fontWeight: 500 }}>{b.name}</div>
              <div style={{ ...TYPE.sans, fontSize: 10, color: theme.textMuted, marginTop: 2,
                lineHeight: 1.3 }}>{b.desc}</div>
              {!b.unlocked && b.progress != null && (
                <div style={{ marginTop: 8, padding: '0 8px' }}>
                  <div style={{ height: 3, background: theme.borderStrong, borderRadius: 2 }}>
                    <div style={{ width: `${b.progress * 100}%`, height: '100%',
                      background: theme.accent, borderRadius: 2 }} />
                  </div>
                  <div style={{ ...TYPE.mono, fontSize: 9, color: theme.textMuted, marginTop: 4 }}>
