// ONBOARDING — multi-step goal setup

function OnboardingScreen({ tweaks, theme, nav, setTweak, app }) {
  ...
  return (
    <div data-screen-label="08 Onboarding" style={{
      background: theme.bg, color: theme.text, minHeight: '100%',
      display: 'flex', flexDirection: 'column',
    }}>
      {/* Top: progress + skip */}
      <div style={{ padding: '14px 18px 8px', display: 'flex',
        ...
        <button onClick={() => { setTweak('hasOnboarded', true); nav('home'); }}
          style={{ background: 'transparent', border: 'none',
            color: theme.textMuted, ...TYPE.sans, fontSize: 12, cursor: 'pointer' }}>
          Skip
        </button>
      </div>
      ...
      <div style={{ flex: 1, padding: '20px 22px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ ...TYPE.display, fontSize: 32, color: theme.text, marginTop: 8, lineHeight: 1.1 }}>
          {cur.title}
        </div>
        <div style={{ ...TYPE.serif, fontSize: 16, fontStyle: 'italic',
          color: theme.textDim, marginTop: 4 }}>
          {cur.subtitle}
        </div>
        <div style={{ flex: 1, marginTop: 24,
          animation: 'slideUp 320ms ease' }} key={step}>
          {cur.body}
        </div>
      </div>
...
function WelcomeStep({ theme }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center',
      justifyContent: 'center', height: '100%', textAlign: 'center', gap: 24 }}>
      <div style={{
      ...
      <div style={{ ...TYPE.serif, fontSize: 16, fontStyle: 'italic',
        color: theme.textDim, maxWidth: 280, lineHeight: 1.5 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%',
        ...TYPE.mono, fontSize: 11, color: theme.textMuted }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
      ...
      <div style={{ ...TYPE.serif, fontSize: 14, fontStyle: 'italic',
        color: theme.textDim, textAlign: 'center', maxWidth: 280, lineHeight: 1.5 }}>
        At a healthy 0.5 kg/week pace, you'll reach this in <strong style={{ color: theme.accent }}>
        {Math.ceil(diff / 0.5)} weeks</strong>.
      </div>
    </div>
  );
}
