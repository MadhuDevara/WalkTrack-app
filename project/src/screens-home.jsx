// HOME / TODAY screen — the hero of the app

function HomeScreen({ tweaks, theme, nav, app }) {
  const { currentSteps, stepGoal, metric, showWeightPanel } = tweaks;
  const distanceKm = (currentSteps * 0.00076).toFixed(2);
  const distance = metric ? `${distanceKm} km` : `${(distanceKm * 0.621371).toFixed(2)} mi`;
  ...
  // Hourly step distribution for sparkline
  ...
  return (
    <div data-screen-label="01 Home" style={{
          ...
          <>
            ...
            <IconButton theme={theme} variant="soft" onClick={() => nav('profile')}>
              <div style={{ width: 28, height: 28, borderRadius: '50%',
                ...
                ...TYPE.sans, fontSize: 12, fontWeight: 600, color: theme.bg }}>M</div>
            </IconButton>
          </>
        }
      />
      ...
      <div style={{ display: 'flex', justifyContent: 'center', padding: '0 22px 18px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, ...TYPE.serif,
          fontSize: 14, color: theme.textDim, fontStyle: 'italic', textAlign: 'center' }}>
      ...
      <div style={{ padding: '0 16px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <StatCard theme={theme} label="Distance" value={metric ? distanceKm : (distanceKm * 0.621371).toFixed(2)}
          ...
          <div style={{ padding: '16px 18px', color: theme.bg,
            ...
            <div>
              <div style={{ ...TYPE.sans, fontSize: 11, letterSpacing: '0.18em',
                textTransform: 'uppercase', opacity: 0.75 }}>Tap to start</div>
              <div style={{ ...TYPE.display, fontSize: 22, marginTop: 2 }}>Begin a walk</div>
              <div style={{ ...TYPE.sans, fontSize: 12, opacity: 0.8, marginTop: 2 }}>
                GPS · pace · live coaching
              </div>
            </div>
            ...
            <div style={{ padding: '16px 16px 14px',
              ...
              <div>
                <div style={{ ...TYPE.sans, fontSize: 11, letterSpacing: '0.16em',
                  textTransform: 'uppercase', color: theme.textMuted }}>Down this week</div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 4 }}>
                  <span style={{ ...TYPE.display, fontSize: 32, color: theme.text }}>−1.2</span>
                  <span style={{ ...TYPE.mono, fontSize: 12, color: theme.textDim }}>{metric ? 'kg' : 'lb'}</span>
                </div>
                <div style={{ ...TYPE.sans, fontSize: 12, color: theme.textDim, marginTop: 2 }}>
                  68.4{metric ? 'kg' : 'lb'} · 9.6 to goal
                </div>
              </div>
              <Sparkline data={[72, 71.8, 71.4, 71.6, 70.9, 70.2, 69.6, 68.4]}
                width={120} height={48} color={theme.accent} theme={theme} />
            </div>
            <div style={{ borderTop: `1px solid ${theme.border}`, padding: '10px 16px',
              display: 'flex', justifyContent: 'space-between', ...TYPE.mono, fontSize: 11,
              color: theme.textDim }}>
              <span><span style={{ color: theme.accent }}>+312</span> deficit today</span>
              <span>{metric ? '4.0kg' : '8.8lb'} since Apr 1</span>
            </div>
          </Card>
        </div>
      )}
...
function WellnessChip({ theme, icon, value, label, tone }) {
  return (
    <div style={{
      ...
      <div style={{ ...TYPE.sans, fontSize: 10, letterSpacing: '0.14em',
